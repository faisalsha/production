...
function static_inq_form_validate(frm) {
	...
	if (frm=="" || frm=="10001") {  //Buisness Inquiry Form
	...
	else if (frm=="10024") {  //EI Product Detail Page Inquiry form
	
		if (chktrim(document.detail_form.dynFrm_details_2.value).length==0 || chktrim(document.detail_form.dynFrm_details_2.value).length<20) {
			
	    	alert("Please Enter Your Requirement Details [ Minimum 20 Characters ]");
	        document.detail_form.dynFrm_details_2.focus();
	        return false;
		}
		
		if (chktrim(document.detail_form.dynFrm_details_2.value).length>1000) {
	    ...
	    if (chktrim(document.detail_form.dynFrm_contact_person.value).length==0) {
		
	    	alert("Please Enter Your Name");
	        document.detail_form.dynFrm_contact_person.focus();
	        return false;
		}
		
		var username = document.detail_form.dynFrm_email_id.value;
		
		if(chktrim(username)=='') {
			
			alert("Please enter email.");
			document.detail_form.dynFrm_email_id.focus();
			return false;
		}
		...
		if (document.detail_form.dynFrm_country.options[document.detail_form.dynFrm_country.selectedIndex].value=="") {
			
		  	alert("Please Select Your Country.");
		    document.detail_form.dynFrm_country.focus();
		    return false;
		}
		
		if (chktrim(document.detail_form.dynFrm_phone.value).length==0) {
	
		    alert("Please Enter Phone/Mobile Number.");
		    document.detail_form.dynFrm_phone.focus();
			return false;
		}
		...
		if(chktrim(document.detail_form.dynFrm_phone.value).length>0) {
		    ...
		    if(isNaN(document.detail_form.dynFrm_phone.value)) {
			
				alert("Please enter valid mobile number.Do not enter alphabet or special characters.");
				document.detail_form.dynFrm_phone.focus();
				return false;
			}
			if(document.detail_form.dynFrm_country.options[document.detail_form.dynFrm_country.selectedIndex].value=="IN^91" && (chktrim(document.detail_form.dynFrm_phone.value).length<10 || chktrim(document.detail_form.dynFrm_phone.value).length>10)) {
				
				alert("Please Enter Your 10 digits mobile number.");
			    document.detail_form.dynFrm_phone.focus();
				return false;
			}
			else if(document.detail_form.dynFrm_country.options[document.detail_form.dynFrm_country.selectedIndex].value!="IN^+91" && (chktrim(document.detail_form.dynFrm_phone.value).length<3 || chktrim(document.detail_form.dynFrm_phone.value).length>15)) {
